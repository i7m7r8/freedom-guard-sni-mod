      ...
      if (sni.trim().isNotEmpty) {
        try {
          ...
          if (sniJson is Map &&
            ...
            String spoofedSNI = sniJson["serverName"].toString();
            LogOverlay.addLog("Applying Real SNI Spoofing: $spoofedSNI");
            ...
            for (var outbound in parsedJson["outbounds"]) {
              if (outbound is Map<String, dynamic>) {
                ...
                if (stream is Map<String, dynamic>) {
                  ...
                  if (security == "tls") {
                    stream["tlsSettings"] ??= {};
                    // Standard SNI override
                    stream["tlsSettings"]["serverName"] = spoofedSNI;
                    // Advanced SNI override for newer Xray cores (decouples SNI from destination)
                    stream["tlsSettings"]["sniOverride"] = spoofedSNI;
                  }

                  if (security == "reality") {
                    stream["realitySettings"] ??= {};
                    stream["realitySettings"]["serverName"] = spoofedSNI;
                    // Reality also supports sniOverride in some forks
                    stream["realitySettings"]["sniOverride"] = spoofedSNI;
                  }
                }
              }
            }
